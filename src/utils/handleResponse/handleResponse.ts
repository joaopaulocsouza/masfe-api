...
export const handleRegisterRes = ({code, res, content}: Props) => {
    switch(code){
        case "RGS-01": 
            res.cookie("token", content.token, {...content.options, sameSite: "lax", secure: true, domain: "onrender.com"}).json({
                code: "RGS-01",
...
export const handleLoginRes = ({code, res, content}: Props) => {
 switch(code){
        case "LGN-01": 
            res.cookie("token", content.token, {...content.options, sameSite: "lax", secure: true, domain: "onrender.com", path: "/"}).json({
                code: "LGN-01",
