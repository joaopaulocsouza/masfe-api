...
export const handleRegisterRes = ({code, res, content}: Props) => {
    switch(code){
        case "RGS-01": 
            res.cookie("token", content.token, {...content.options, sameSite: "strict", secure: true}).json({
                code: "RGS-01",
...
export const handleLoginRes = ({code, res, content}: Props) => {
 switch(code){
        case "LGN-01": 
            res.cookie("token", content.token, {...content.options, sameSite: "strict", secure: true, path: "/"}).json({
                code: "LGN-01",
